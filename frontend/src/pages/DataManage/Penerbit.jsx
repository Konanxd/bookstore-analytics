...
import { getPenerbit } from "../../api";

const formFields = [
  { id: "id_penerbit", label: "ID" },
  { id: "nama_penerbit", label: "nama penerbit" },
];
...
export default function Pelanggan() {
  ...
  useEffect(() => {
    async function fetchData() {
      const dataList = await getPenerbit();
      setData(dataList);
    }
  ...
  const handleAddData = () => {
    if (isEditMode) {
      ...
      updatedData[currentEditIndex] = {
        ...newData,
        id_penerbit: data[currentEditIndex].id,
      };
      setData(updatedData);
      setIsEditMode(false);
    } else {
                  ...
                  {formFields.map((field) => (
                    <div key={field.id} className='mb-4'>
                      ...
                      <input
                        ...
                        disabled={field.id === "id_penerbit" && isEditMode}
                      />
                    </div>
                  ))}
                </div>
