...
import TablePenulis from "../../components/TablePenulis";
import { getPenulis } from "../../api";

const formFields = [
  { id: "id_penulis", label: "ID" },
  { id: "nama_penulis", label: "nama penulis" },
];
...
export default function Pelanggan() {
  ...
  useEffect(() => {
    async function fetchData() {
      const dataList = await getPenulis();
      setData(dataList);
    }
  ...
  const handleAddData = () => {
    if (isEditMode) {
      ...
      updatedData[currentEditIndex] = {
        ...newData,
        id_penulis: data[currentEditIndex].id,
      };
      setData(updatedData);
      setIsEditMode(false);
    } else {
        ...
        <div className='relative flex flex-col flex-1 overflow-y-auto overflow-x-hidden'>
          ...
          <main>
            <div className='px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto flex flex-col gap-3'>
              ...
              <TablePenulis data={data} onEdit={handleEditData} onDelete={handleDeleteData} />
            </div>
          </main>
                  ...
                  {formFields.map((field) => (
                    <div key={field.id} className='mb-4'>
                      ...
                      <input
                        ...
                        disabled={field.id === "id_penulis" && isEditMode}
                      />
                    </div>
                  ))}
                </div>
