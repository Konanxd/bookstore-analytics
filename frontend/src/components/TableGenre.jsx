...
export default function TableCustomer({ data, onEdit, onDelete }) {
  ...
  const sortedData = React.useMemo(() => {
    ...
    return [...data].sort((a, b) => {
      if (sortConfig.key === "id_genre") {
        return sortConfig.direction === "ascending"
          ? a[sortConfig.key] - b[sortConfig.key]
          : b[sortConfig.key] - a[sortConfig.key];
      }

      if (["nama_genre"].includes(sortConfig.key)) {
        return sortConfig.direction === "ascending"
        ...
        {sortedData.map((row, index) => (
          ...
          >
            {tableFields.map((field) => (
              <td key={`${row.id_genre}-${field}`} className={commonCellClass}>
                {row[field]}
              </td>
            ))}
            <td className={`${commonCellClass} relative`}>
              <div className='flex justify-center'>
                ...
                {activeDropdown === index && <SettingsDropdown rowIndex={index} />}
              </div>
            </td>
          </tr>
        ))}
...
TableCustomer.propTypes = {
  data: PropTypes.arrayOf(
    PropTypes.shape({
      id_genre: PropTypes.number.isRequired,
      nama_genre: PropTypes.string.isRequired,
    })
  ).isRequired,
